...
Generated by `scripts/10_metric_unification.py` from `results/raw/*.json`. Every aggregate is rank-based: `Infidelity` spans fifteen orders of magnitude across methods, so means and Pearson correlations over it are reports on `Deconvolution`'s tail.
...
**FaithfulnessCorrelation, MonotonicityCorrelation** put under 10 % of their variance on the method. Differences between adjacent methods on those columns of `results/RESULTS.md` are not resolved by n = 200, and the marginal `mean +/- std` presentation hides that.
...
Surviving Bonferroni over the 15 pairs: **Infidelity~RandomLogit**.
...
Ranks are oriented so higher = better, so a positive loading means the component agrees with that metric's own notion of quality.
